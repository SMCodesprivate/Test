...
export default function Curses() {
    ...
    async function init() {
        ...
        curses.map(async curse => {
            ...
            // VANTAGENS
            var vantagens = document.createElement("div");
            var vantagem = document.createElement("h1");
            var vantagemText = document.createTextNode("Vantagens:");
            var vants = document.createElement("ul");
            vantagens.setAttribute("class", "vantagens");

            // BUTTON
            var button = document.createElement("div");
            var buttonText = document.createTextNode("Access course");
            button.setAttribute("class", "botaoCadidatar_"+curse.name.toLowerCase());
            ...
            vantagem.appendChild(vantagemText);
            vantagens.appendChild(vantagem);
            curse.benefits.map(benefit => {
                var vant = document.createElement("li");
                var vantText = document.createTextNode(curse.benefits.indexOf(benefit)+1+". "+benefit);
                vant.appendChild(vantText);
                vants.appendChild(vant);
                vantagens.appendChild(vants);
            });
            list.appendChild(vantagens);
